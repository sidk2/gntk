    ...
    for d in dataset:
        ...
        for i, n_mlp in enumerate(mlp_layers):
            for j, n_block in enumerate(block_ops):
                ...
                t3 = time.time()
                cmd = ['python', 'gram_node.py', '--dataset', f'{d}', '--num_mlp_layers', f'{n_mlp}', '--num_layers', str(n_block), '--scale', 'degree', '--out_dir', 'out/', '--jk', '0', '--type', 'GCN', '--skip', '1'] 
                output = subprocess.run(cmd, capture_output=True, text=True)
